...
// create companies
export const createCompanies = async (companies) => {
  try {
    const { data } = await axios.post(
      `${server_url}/api/company/create`,
      { companies },
      { headers }
    );
    return data.companies;
...
// updating a company
export const updateCompany = async (company_id, company_name) => {
  try {
    const { data } = await axios.put(
      `${server_url}/api/company/${company_id}`,
      { name: company_name }
    );
    return data.message;
  } catch (error) {
