...
import { getProblemById, editProblem, uploadToS3 } from "@/api/problemApi"
import { languages } from "@/assets/mapping"

export function EditProblem() {
    const navigate = useNavigate();
    const { id } = useParams(); // No type param
        ...
        try {
            ...
            const formData = {
                ...
                testCases: data.testCases
                    .filter((testCase) => testCase.input !== null && testCase.output !== null)
                    .map((testCase) => testCase.isExample),
            };
                                            ...
                                            <div key={type}>
                                                ...
                                                <div className="relative">
                                                    <input
                                                        type="file"
                                                        id={`${type}-${index}`}
                                                        onChange={(e) => handleTestCaseChange(index, type, e.target.files?.[0] || null)}
                                                        className="hidden"
                                                    />
                                                    <Button
                                                        type="button"
                                                    ...
                                                    >
                                                        ...
                                                        {testCase[type] ? testCase[type].name : `Upload ${type}`}
                                                    </Button>
                                                </div>
                                            </div>
                                        ))}
