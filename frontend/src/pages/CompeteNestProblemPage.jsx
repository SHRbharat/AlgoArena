...
import { getProblemById, getAllExampleTestcases, getFileData, getProblemSubmissions } from "@/api/problemApi"
import { useAppDispatch, useAppSelector } from "@/redux/hook"
import { setProblem, setExampleTestcases, setSubmissions } from "@/redux/slice/problemSlice"
...
export function CompeteNestProblemPage() {
  ...
  useEffect(() => {
    const fetchProblem = async () => {
      try {
        setIsLoading(true)
        const problemData = await getProblemById(problem_id)
        ...
        const testcaseURLs = await getAllExampleTestcases(problem_id)
        const input = await Promise.all(testcaseURLs.input_urls.map((url) => getFileData(url)))
        const exp_output = await Promise.all(testcaseURLs.output_urls.map((url) => getFileData(url)))
        dispatch(setExampleTestcases({ input, exp_output }))

        // fetching all the submissions of the problem
        const submissions = await getProblemSubmissions(problem_id)
    ...
    <div className="h-[calc(100vh-3.5rem)]">
      {contest && (
        <div className="px-6 py-2 flex justify-between items-center flex-wrap gap-4">
          ...
          {getContestStatus() !== "Ended" ? (
            ...
            <Badge className="text-md px-4 py-2 text-red-500">Ended</Badge>
          )}
        </div>
      )}
        ...
        <ResizablePanel defaultSize={50} minSize={30}>
          <div className="relative h-full">
            <div className={`${isFullScreen ? "fixed inset-0 z-50 bg-background" : "h-full"}`}>
              <CodeEditor handleTab={setCurrentTab} isFullScreen={isFullScreen} handleFullScreen={setIsFullScreen} />
            </div>
          </div>
        </ResizablePanel>
      </ResizablePanelGroup>
      ...
      <Dialog open={showEndedPopup} onOpenChange={setShowEndedPopup}>
        <DialogContent>
          ...
          <DialogDescription className="text-lg">The contest has ended.</DialogDescription>
