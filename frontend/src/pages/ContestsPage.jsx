...
import { getAllContests, handleRegistration } from "@/api/contestApi"
...
export function ContestsPage() {
  ...
  const handleRegister = (contestId, isRegister) => {
    handleRegistration(contestId, isRegister)
      .then((_data) => {
        toast.success(_data.message)
        setContests((prevContests) =>
          prevContests.map((contest) =>
            contest.id === contestId ? { ...contest, attended: isRegister } : contest
          )
        )
      })
      .catch((err) => {
              ...
              <div className="grid gap-6 md:grid-cols-2 lg:grid-cols-3">
                {upcomingContests.map((contest) => (
                  <ContestCard
                    ...
                    contest={contest}
                    isPast={false}
                    contestRegister={handleRegister}
                  />
                ))}
              </div>
            ) : (
